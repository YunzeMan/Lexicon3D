...
class Blip2T5(Blip2Base):
    ...
    def __init__(
        ...
        self.visual_feature_channel = visual_feature_channel # added by Yunze
        
        self.Qformer3D, self.query_tokens = self.init_Qformer(num_query_token, self.visual_feature_channel)
        self.Qformer3D.cls = None
        self.Qformer3D.bert.embeddings.word_embeddings = None
        self.Qformer3D.bert.embeddings.position_embeddings = None
        for layer in self.Qformer3D.bert.encoder.layer:
            layer.output = None
        ...
        self.t5_proj = nn.Linear(self.Qformer3D.config.hidden_size, self.t5_model.config.hidden_size)

        pos_model = PositionalEncoding1D(self.visual_feature_channel // 3)
        x = torch.zeros(1, 512, self.visual_feature_channel // 3) # used to be (1, 256, 1408//3), changed to (1, 512, visual_feature_channel//3)
    ...
    def forward(self, samples):
        with torch.cuda.amp.autocast(dtype=torch.float32):
            pc_embeds = samples["pc_feat"] # [B, 5000(N_points), 1408(feature_channel)]
        ...
        with torch.cuda.amp.autocast(dtype=torch.float32):
            ...
            for j in range(pc.shape[0]):
                ...
                pcs = torch.cat(pcs, -1)
                all_pcs[j][:, :(3*(self.visual_feature_channel//3))] = pcs
        ...
        query_output = self.Qformer3D.bert(
    ...
    def from_config(cls, cfg):
        ...
        visual_feature_channel = cfg.get("visual_feature_channel", 1408)
        ...
        model = cls(
            ...
            visual_feature_channel=visual_feature_channel,
        )
