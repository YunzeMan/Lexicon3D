...
class VjepaFeature(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        feature_file = os.path.join(self.data_dir, "lexicon3d", "vjepa_unified_pairs", "feat_xyz_overlap_transformed", scene_name + ".pt")
        pose_file = os.path.join(self.data_dir, "lexicon3d", "unified_pose", scene_name + ".pt")

        feat_xyz_overlap = torch.load(feature_file)
        feat_1 = torch.from_numpy(feat_xyz_overlap['feat_1']).float()
        feat_2 = torch.from_numpy(feat_xyz_overlap['feat_2']).float()
